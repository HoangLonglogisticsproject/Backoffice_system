...
describe('canonical email identity (§1, §4, §9)', () => {
  ...
  describe('creating an account directly', () => {
    ...
    it('signs in with ANY spelling of the address — the server canonicalises', async () => {
      ...
      const response = await login(client, `${local.toUpperCase()}@HOANGLONGTI.COM`, 'temp pass 1');
  ...
  describe('raising an invitation', () => {
    ...
    it('★ the queue carries one row per person, not one per spelling', async () => {
      const queue = await boss.get('/account-invitations', { params: { limit: 200 } });
      expect(queue.status).toBe(200);

      const spellings = (queue.data.items as { email: string }[])
        .map((row) => row.email)
