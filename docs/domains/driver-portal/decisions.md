...
| # | Quyết định | Trạng thái | Enforce |
|---|---|---|---|
| **DL-114** | Driver Portal có **shell riêng, mobile-first** (`DriverLayout`): thanh trên + điều hướng 3 đích **thật** (lịch làm việc · thông báo · hồ sơ) cố định dưới đáy điện thoại, nằm trong thanh trên từ `md`. **Không** dùng `AppShell` (sidebar/drawer của Backoffice) — đúng contract §1 "giao diện riêng". Chung với Backoffice: design token, `StatusPill`, `isNavActive`. Web responsive trên trình duyệt điện thoại — **không** native app, **không** PWA | **[CONFIRMED]** D1 brief 2026-09-25 | `App.driver.spec.tsx` |
| **DL-115** | Lịch làm việc: **một thẻ cho mỗi assignment**, mở bằng `assignmentId` — thay cho "nhóm theo trip, một link mỗi xe" của ADR-0004 §5 (ADR accepted, không sửa); biển số trên thẻ phân biệt hai lượt của cùng một trip. Tab **Hôm nay / Sắp tới / Đã qua** chia theo `scheduledOn` so với ngày nghiệp vụ Asia/Ho_Chi_Minh, **không** theo trạng thái: `GET /driver/assignments` không mang event / completion / status (contract §5.4.1). Vì vậy **không** có tab "Đã hoàn thành" và **không** có badge trạng thái trên thẻ cho tới khi list có dữ liệu đó (phụ thuộc DL-69, DL-71) | **[CONFIRMED]** D1 brief + hệ quả contract §5.4.1 | `DriverWorkflow.spec.tsx` · `driverSchedule.spec.ts` |
| **DL-116** | Trạng thái trên màn chi tiết là **của assignment**, derive từ event + completion của chính lượt đó (`assignmentStatusOf`): Đã phân công → Đang ở điểm lấy hàng → Đang vận chuyển → Đang ở điểm giao hàng → Chờ gửi hoàn tất → Chờ duyệt / Bị trả lại / Đã duyệt. Completion outrank hành trình. **Không** "trễ" (ngưỡng SLA — design.md O-4 — còn mở), **không** dispatch status (DL-69), không persist | **[CONFIRMED]** | `driverExecution.spec.ts` |
