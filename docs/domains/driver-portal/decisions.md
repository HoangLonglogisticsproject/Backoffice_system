...
| **Trạng thái** | `CONFIRMED` · `CEO DECISION` · `WORKIK` · `GAP` · `FUTURE` · `SUPERSEDED → DL-nn` (rule đã bị decision sau thay thế toàn bộ; giữ làm lịch sử) · `PARTIALLY SUPERSEDED → DL-nn` (phần nêu rõ bị thay, phần còn lại vẫn hiệu lực) |
...
| # | Quyết định | Trạng thái | Enforce |
|---|---|---|---|
| **DL-101** | Sales / Kế toán / Điều độ **không** là role. Phân biệt bằng `departments.function ∈ {sales, accounting, dispatch, NULL}`, SuperAdmin đặt qua `PATCH /departments/:id`. Không seed, không đặt tên phòng trong code | **[PARTIALLY SUPERSEDED → DL-113]** — tập function nay là `{sales, accounting, dispatch, customer_service, NULL}` (0033); phần "không là role · không seed · không đặt tên phòng trong code · SuperAdmin đặt qua API" vẫn hiệu lực | `0032` + `PermissionRequirement.orFunction` |
...
| **DL-106** | `trip.create` **không** còn `any`: SuperAdmin hoặc phòng `sales` / `accounting` / `dispatch` (member lẫn head). Phòng khác (Marketing, HR, IT…) đọc board, không tạo Trip; Driver bị chặn ở boundary. Cùng key gate thêm xe / khách / địa điểm | **[PARTIALLY SUPERSEDED → DL-112 · DL-113]** — `trip.create` không còn gate xe/khách/địa điểm (DL-112) và có thêm `customer_service` (DL-113); phần "không `any`, phòng khác không tạo Trip, Driver bị chặn ở boundary" vẫn hiệu lực | `PERMISSION_REQUIREMENT` + `authorization-policy.spec.ts` + trip-schedule security spec |
| **DL-109** | Dữ liệu Trip (board, chi tiết, giá, danh mục xe/khách/địa điểm) chỉ thuộc phòng `sales` / `accounting` / `dispatch` và SuperAdmin. `trip.read` và `trip.price.read` = `global ∨ function` (bỏ `any` / `head-anywhere`); `trip.write` = head **trong** phòng chức năng (`withinFunction`). `DEPARTMENT_HEAD` phòng khác **không** tự có trip visibility. **Customer Service: deferred**, chưa có function, không suy đoán | **[PARTIALLY SUPERSEDED → DL-111 · DL-112 · DL-113]** — giá chỉ của Kế toán (DL-111), danh mục tách key riêng (DL-112), Customer Service không còn deferred (DL-113); phần "`trip.read` = global ∨ function, `trip.write` = head trong phòng chức năng, head phòng khác không có visibility, frontend ẩn menu khi thiếu `trip.read`" vẫn hiệu lực | `PERMISSION_REQUIREMENT` + `authorization-policy.spec` + trip-schedule / trip-completion security spec; frontend ẩn mục ĐIỀU PHỐI và 3 màn hình khi thiếu `trip.read` |
| **DL-111** | **Giá là của Kế toán**: `trip.price.read` = `trip.price.write` = `global ∨ accounting`. Sales / Customer Service / Điều phối tạo Trip **không giá** (gửi key giá → 403, không strip), Kế toán bổ sung qua PATCH chỉ-key-giá; Kế toán/SA `POST` bắt buộc `sellPrice`, `purchasePrice` tuỳ chọn. `driver.account.request` = `global ∨ dispatch` (bỏ `head-anywhere`); SA duyệt (`user.write`). `trip.complete.review` vẫn global-only | **[CONFIRMED]** CEO 2026-09-18 | ADR-0006; `PERMISSION_REQUIREMENT`; `trip-schedule.security.spec` matrix; `driver-account.security.spec` |
| **DL-112** | **Master data tách khỏi `trip.create`**: `customer.create` · `location.create` = `global ∨ {sales, accounting, dispatch, customer_service}`; `vehicle.create` = `global ∨ dispatch`. `trip.create` chỉ còn `POST /trip-schedules`. Sửa/archive catalogue vẫn `trip.write` (ownership sửa chưa redesign) | **[CONFIRMED]** CEO 2026-09-18 | ADR-0006; `trip-catalogue.controller`; frontend `TripMasterDataPage` / `LocationCataloguePage` / `TripFormModal` / `CatalogueSelect` |
| **DL-113** | **Customer Service là function riêng** `customer_service` (migration `0033` mở CHECK, không seed, `0032` giữ nguyên). CS: `trip.read`, `trip.create`, `customer.create`, `location.create`; **không** giá, **không** `vehicle.create`, **không** `dispatch.write`, **không** `trip.write`. Thay thế mục "CS deferred" ở DL-109 | **[CONFIRMED]** CEO 2026-09-18 | ADR-0006; `DEPARTMENT_FUNCTIONS`; `0033_department_function_customer_service.sql` + schema spec |
| **DL-110** | `departments.function` **được giữ** (ADR-0005). Các trường của Department: `id`/`slug`/`name` là trường **định danh** (`slug` là định danh URL do người tạo gõ, bất biến, không phải phân loại nghiệp vụ; `name` đổi tự do), `status` là trường **vòng đời**, `function` là trường **phân loại chức năng nghiệp vụ**. Không authorize bằng `name`/`slug`. `function` là **cấu hình do SuperAdmin sở hữu**: đặt được ngay khi tạo (`POST /departments { function }`) hoặc sau (`PATCH`), `null` = phòng thường. Không seed, không default, không suy từ tên. Production: SuperAdmin `PATCH` ba phòng Sales/Kế toán/Điều độ một lần — cấu hình, không phải migration | **[CONFIRMED]** 2026-09-17 | `DepartmentService.create/update`, `organization.security.spec`, `organization.integration.spec`, contract §5 |
...
# 12. ★ Bổ sung 2026-09-25 — Driver Portal D1: web mobile

| # | Quyết định | Trạng thái | Enforce |
|---|---|---|---|
| **DL-114** | Driver Portal có **shell riêng, mobile-first** (`DriverLayout`): thanh trên + điều hướng 3 đích **thật** (lịch làm việc · thông báo · hồ sơ) cố định dưới đáy điện thoại, nằm trong thanh trên từ `md`. **Không** dùng `AppShell` (sidebar/drawer của Backoffice) — đúng contract §1.1 "giao diện riêng". Tách **source/application** riêng (contract §1, design §A-8.6 `[REC]`) **chưa** quyết ở đây — vẫn một SPA, xem DL-07. Phạm vi D1: web responsive trên trình duyệt điện thoại — **không** native app, **không** PWA; offline / evidence vẫn chờ DL-88 | **[CONFIRMED]** — hệ quả contract §1.1 · phạm vi D1 2026-09-25 | `App.driver.spec.tsx` |
| **DL-115** | Lịch làm việc: **một thẻ cho mỗi assignment**, mở bằng `assignmentId` (ADR-0004 R-13, §6); biển số trên thẻ phân biệt hai lượt của cùng một trip. Chỉ đổi **phần trình bày** "nhóm theo trip, một link mỗi xe" mà ADR-0004 §5 ghi lúc ship — không đổi quyết định nào của ADR (ADR accepted, không sửa). Tab **Hôm nay / Sắp tới / Đã qua** chia theo `scheduledOn` so với ngày nghiệp vụ Asia/Ho_Chi_Minh (contract §10.6), **không** theo trạng thái: `GET /driver/assignments` hiện không mang event / completion (hình dạng API) và không mang dispatch status (contract §5.4.1). Vì vậy **không** có tab "Đã hoàn thành" và **không** có badge trạng thái trên thẻ cho tới khi list có dữ liệu đó (phụ thuộc DL-69, DL-71). "Đã qua" chỉ là ngày, hiện đúng những gì list trả — **không** quyết DL-71 | **[CONFIRMED]** — hệ quả ADR-0004 R-13 · contract §10.6 · phạm vi D1 2026-09-25 | `DriverWorkflow.spec.tsx` · `driverSchedule.spec.ts` |
| **DL-116** | Trạng thái trên màn chi tiết là **của assignment**: nhãn hiển thị **suy ra** từ event + completion của chính lượt đó (contract §6 "execution state suy ra từ chuỗi event"; ADR-0004 R-8): Đã phân công → Đang ở điểm lấy hàng → Đang vận chuyển → Đang ở điểm giao hàng → Chờ gửi hoàn tất → Chờ duyệt / Bị trả lại / Đã duyệt. Completion outrank hành trình. Chỉ để hiển thị: không phải status enum, không persist, **không** phải Driver Execution Lifecycle contract §7.3 `[PROPOSED]` — không quyết §7.4 hay DL-31. **Không** "trễ" (ngưỡng SLA — design.md O-4 — còn mở), **không** dispatch status (DL-69) | **[CONFIRMED]** — hệ quả contract §6 · ADR-0004 R-8 | `driverExecution.spec.ts` (`assignmentStatusOf`) |
