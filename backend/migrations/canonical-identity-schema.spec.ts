...
describe('0010_canonical_email_identity.sql', () => {
  ...
  it('★ makes the pending-invitation rule case-insensitive', () => {
    expect(code()).toContain(
      'CREATE UNIQUE INDEX IF NOT EXISTS uq_pending_invitation_email_canonical ON account_invitations (lower(btrim(email))) WHERE status = \'pending\'',
    );
  });
  ...
  it('★ makes the local identity rule case-insensitive', () => {
    expect(code()).toContain(
      'CREATE UNIQUE INDEX IF NOT EXISTS uq_local_identity_subject_canonical ON identities (lower(btrim(subject))) WHERE provider = \'local\'',
    );
  });
  ...
  it('canonicalises with the SAME expression the application uses', () => {
    // `normalizeEmail` and `normalizeSubject` are both trim-then-lowercase. An
    // index that disagreed would look right in isolation and be wrong in pairs.
    expect(code()).toContain('lower(btrim(email))');
    expect(code()).toContain('lower(btrim(subject))');
  });
